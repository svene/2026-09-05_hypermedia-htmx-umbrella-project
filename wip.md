...
WP-T10 + WP-T11 in the docs-project repo; **WP-T6 = two commits** (Thymeleaf repo
+ docs-project repo); WP-T12 spans every pattern project.
...
| WP-T6 | thymeleaf + docs project | **DONE 2026-09-06** | **Thymeleaf pages in the docs site** (the Thymeleaf slice of WP-T11). Two commits: <br> **Part A — thymeleaf repo:** added `docs:start`/`docs:end` markers to the fragments the docs pages show — `fragments/helloworld.html`, `helloworldparams.html`, `helloworldcontent.html`, `m04-parent.html`, `m04-child.html`, `m04-forwarder-first.html`, `m04-forwarder-second.html` (tag `component`); `layout.html`, `page-head.html`, `m03d04-layout.html` (tag `page`; markers placed as siblings of the `th:fragment` element, or as `<!--/* */-->` where they'd otherwise be inside it, so nothing leaks into rendered pages — re-verified all routes: still exactly the 2 intentional page-level markers, no fragment markers in output). Controller `.java` snippets deliberately **not** added (Thymeleaf controllers are trivial `return "view"`; a follow-up if wanted). <br> **Part B — docs-project repo:** `extract-snippets/extract-thymeleaf-snippets.js` (srcRoot `../../2025/2025-08-23_ssfe-patterns-thymeleaf-htmx`, 24 snippets → `generated/snippets/thymeleaf/`), wired into `extract-all-snippets.js`; `src/content/docs/technologies/02_Thymeleaf/m01.mdx`,`m03.mdx`,`m04.mdx`,`m05.mdx` (modelled on the Hono `mNN` pages — iframe + `[Demo]` on `localhost:8080` + snippet components), `s01.mdx` stub removed. **Page naming = `mNN`** (matches the variant's module numbers *and* the back-links the demos already carry — no reconcile needed). **Verified:** `npm run extract-snippets` clean (no "Could not create snippet"); `npm run build` green — all 4 pages built at `/technologies/02_thymeleaf/m0X/`, sidebar auto-picked them up, code blocks + iframes render. Per-demo anchors (`#1-…`) exist on the pages but the in-app back-links stay module-level for now (optional polish). |
...
| WP-T11 | docs project | TODO | **Every pattern variant covered in the docs site.** Coverage so far: `01_JTE` (`s01`–`s06`) = the **JTE-VC** project's sections (confirmed: `@app` alias → `…jte-vc-htmx`, so `01_JTE` is misnamed — it is JTE-VC, incl. the plain-JTE `s01` section and the `s06` experiments); `03_Hono` (`m01`–`m05`); `02_Thymeleaf` (`m01/m03/m04/m05`, done in WP-T6). Remaining: **Qute** (WP-T10); **plain JTE** `2025-08-23_ssfe-patterns-jte-htmx` — decide if it needs its own pages/extractor or is adequately represented by JTE-VC's `s01`. Decide the sidebar taxonomy (`0N_Name` dirs, and whether to rename `01_JTE` → `01_JTE-VC` + add a Qute entry) and page-naming (`sNN` vs `mNN` — Thymeleaf and Hono use `mNN`, JTE-VC uses `sNN`). JSX / Spring-Hono / Graal-JSX rows are out of scope — pattern-course variants only. Depends on WP-T6 (done) + WP-T10; this row is the "nothing left uncovered" check + the taxonomy decision. |
